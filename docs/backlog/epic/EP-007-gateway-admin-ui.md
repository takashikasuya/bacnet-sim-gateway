...
ゲートウェイ管理者向けの軽量 Web UI を、既存 REST 制御プレーン（`PR-F-050`）と**同一プロセス・同一ポート**で提供し、点リスト再読込・モード/管理デバイス/オブジェクト確認・値/状態変更・ログ確認・南向きバインディング確認・北向き BACnet/IP 稼働状態確認を 1 画面で完結させる。サーバレンダリング（FastAPI + Jinja2、自動更新は素の JavaScript `fetch`、Node ビルド非依存）で Raspberry Pi ネイティブ実行（[[ADR-008]]）と最小依存を維持する。北向きは BACnet/IP の稼働状態確認のみとし、上流（Hono/Ditto/Building OS）へはプローブしない（[[ADR-005]]）。

## Acceptance Criteria（対応 AC / 要求）

- [ ] 軽量サーバレンダリング Web UI を提供（Jinja2 + 素の JS fetch、Node 非依存・Pi ネイティブ）（PR-F-052 / [[ADR-008]]）
...
- [ ] 新規ランタイム依存は jinja2 のみ・クライアント JS ライブラリ非依存（素の fetch のみ）（AGENTS.md §5 で明示）
