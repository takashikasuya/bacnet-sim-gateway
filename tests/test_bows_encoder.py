...
def test_analog_entry_matches_schema():
    ...
    assert entry["BACnetObject"] == {
        "_base": "AnalogInput",
        "_value": {"ObjectType": 0, "InstanceNo": 1001},
    }
...
def test_present_value_is_numeric_per_type():
    def rd(ot, v):
        return encode_device_message("d", 1, [Reading(ot, 1, v)], now=NOW)[0]["ValueString"][0][
            "Properties"
        ]["PresentValue"]
