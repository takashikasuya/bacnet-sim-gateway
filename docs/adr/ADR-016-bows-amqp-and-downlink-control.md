...
2. **下り制御は BOWS の consumer パスとして追加**する。
   > **⚠ Superseded by [[ADR-017]]（#67）.** Building OS が下り経路を gRPC `GatewayEgress`
   > として設計したため、本 Decision 2（`Transport.subscribe` + `ControlSchema` による MQTT/AMQP
   > コマンドチャネル、および `POST /api/device-control` 前提）は ADR-017 の gRPC 双方向 stream
   > クライアントに**置換**された。値の型変換は ADR-017 の表に集約。Decision 1（AMQP 1.0 telemetry,
   > #48）は有効のまま。
