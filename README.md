...
- **Gateway モード** — 南向き（MQTT / ZeroMQ / WoT / gRPC）の実データを BACnet オブジェクト化して北向きへ公開
...
```
   ...
   └──────────────────────────────────────────────┘
                  ▼   MQTT / ZeroMQ / WoT / gRPC    ← SOUTHBOUND（バインディング）
...
| 不変条件 | 意味 | 根拠 |
|----------|------|------|
| **北向き = BACnet/IP、南向き = MQTT/ZeroMQ/WoT/gRPC** | MQTT は B-BC の北向き出力ではない | ADR-005 |
...
| `gateway` | 南向きから受信した実データ | 必須（`--transport`） | プロトコル変換ハブ |
...
- `--transport <uri>` — 南向きトランスポート（`mqtt://host:port`, `zmq://...`）
...
> **セキュリティ:** MVP は localhost/LAN 前提・**認証なし**（REST/UI とも `127.0.0.1` バインド）。
> 外部公開・認証は将来 EPIC。信頼できないネットワークに晒さないでください。
...
```bash
uv sync --extra dev              # dev ツール込みで同期
